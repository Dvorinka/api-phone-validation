...
git clone https://github.com/your-username/api-phone-validation.git
cd api-phone-validation
...
- **Base URL**: http://localhost:8080/v1/phone/
...
```bash
curl -H "Authorization: Bearer dev-phone-key" \
     http://localhost:8080/v1/phone/endpoint
```
...
curl -H "X-RapidAPI-Proxy-Secret: your-secret" \
     ...
     https://your-api.p.rapidapi.com/v1/phone/endpoint
```
...
```bash
# Build image
docker build -t api-phone-validation .

# Run container
docker run -p 8080:8080 \
  -e PHONE_VALIDATION_API_KEY=your-key \
...
2. Repository: `your-username/api-phone-validation`
...
| `PHONE_VALIDATION_API_KEY` | Service API key | dev-phone-key |
...
PHONE_VALIDATION_API_KEY=your-development-key
```
...
PHONE_VALIDATION_API_KEY=your-production-api-key
...
2. **Docker Build Issues**
   ...
   docker build --no-cache -t api-phone-validation .
   ```

3. **Environment Issues**
   ```bash
   # Check environment variables
   docker-compose logs phone-validation-api
   ```
